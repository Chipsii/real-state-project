...
export const createNewListing = async (input) => {
...
export async function listListings(q) {
    ...
    if (q.minPrice || q.maxPrice) {
        filter.price = {};
        if (q.minPrice)
            filter.price.$gte = Number(q.minPrice);
        if (q.maxPrice)
            filter.price.$lte = Number(q.maxPrice);
    }
    ...
    if (q.minSqft || q.maxSqft) {
        filter.sqft = {};
        if (q.minSqft)
            filter.sqft.$gte = Number(q.minSqft);
        if (q.maxSqft)
            filter.sqft.$lte = Number(q.maxSqft);
    }
    if (q.tags) {
...
export async function updateListingById(id, payload) {
    ...
    const updated = await Listing.findByIdAndUpdate(id, { $set }, { new: true, runValidators: true });
    if (!updated) {
        throw new AppError("Listing not found", 404);
    }
    return updated;
